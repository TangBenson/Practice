...
// /*
// 靜態的成員不需要實體(Instance)就能進行訪問
// 非靜態的成員必須 new 一個實體(Instance)才能進行訪問

// 靜態並非沒有實體 , 而是只有一個實體 , 在程式執行之初就建立 , 並佔用記憶體位置 , 而且一直存在 , 當程式用上一堆靜態成員的時候就造成不必要的記憶體浪費

// 靜態方法屬於類別所有，只能以類別名叫用，即使是非靜態類別
// 非靜態方法屬於實體所有，只能以實體叫用 ，所以靜態類別內無法有非靜態方法(靜態類別內一切都是靜態的，不僅限方法)
...
//         NotStaticClass b = new NotStaticClass();
//         //網路文章說一個設1則全部都1，我聽他放屁....
//         a.ID = 50;
//         Console.WriteLine(a.ID);
//         Console.WriteLine(b.ID);
//         NotStaticClass.ID2 = 100;
//         Console.WriteLine(NotStaticClass.ID2);
